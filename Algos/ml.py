...
class ML():
    def __init__(self,windows_len,predict_len,epoches,discount_factor,step_size,lr,device,save_every,display_every):
        self.device = device
        self.LR = lr
    ...
    def train(self, train_data_loader, model_save_folder_path, task=None, model_path=None, **kwargs):
        ...
        for epoch in range(self.EPOCHS):
            ...
            if (epoch + 1) % self.SAVE_EVERY == 0:
                save_path = cfg.ml_model_path(folder=model_save_folder_path,model_name=algo_name,cuda=self.device,epoch=epoch+1,lr=self.LR)
    ...
    def test_task(self, test_data_loader, model_path,task,data_range):
        ...
        for epoch in range(1):
            ...
            print(f'  | Epoch [{epoch + 1}/1] Avg MSE Loss | {avg_loss} Avg Task Regret | {avg_task_loss} Avg Task Norm Regret | {avg_norm_task_loss}' )
        print(f'=============== {algo_name} Testing End ===============')
